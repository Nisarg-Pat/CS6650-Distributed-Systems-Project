...
const BookItem = ({book, type}) => {
    ...
    const onClickSellBook = () => {
        ...
        dispatch(sellBookThunk({book:newBook, url:server.url}));
    }

    const onClickShelfBook = () => {
        dispatch(shelfBookThunk({book, url:server.url}));
    }

    const onClickDeleteBook = () => {
        dispatch(deleteBookThunk({book, url:server.url}));
    }
    ...
    const onClickAddToCartBtn = async () => {
        if(!currentUser) {
            ...
            await dispatch(addToCartThunk({cart, url: server.url}))
            navigate("/cart");
        }
    }
    ...
    const onClickRemoveFromCartBtn = () => {
        const cart = {userId: currentUser.id, bookId: book.id}
        dispatch(removeFromCartThunk({cart, url:server.url}))
    }
